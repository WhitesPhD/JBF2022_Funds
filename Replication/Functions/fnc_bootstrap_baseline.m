function [mxBSAlpha, mxBSAlphaTstat, mxBSBetas, mxBSBetasTstat] = fnc_bootstrap_baseline(vecY,mxX,id,time,b)

% This function implements a baseline bootstrap specification
...
for ifund = 1:1:max(id)
        display(['Fund number ', num2str(ifund)])
    for i = 1:b
        
        [resYResample(id==ifund,i),mxIndex(id==ifund,i)]    = datasample(resY(id==ifund),length(resY(id==ifund)));
        
        mxX_temp                        = mxX(id==ifund,:);
        mxXResample(id==ifund,:,i)      = mxX_temp(mxIndex(id==ifund,i),:);
...
mxBSAlphaTstat = zeros(b,length(unique(id)));       % Alpha tstat
mxBSBetasTstat = zeros(b,size(mxX,2));              % Beta tstat
